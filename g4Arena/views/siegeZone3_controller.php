...
if (isset($_POST['envoyer'])) {
    ...
    header('Location:/ressources/views/siegeZone3.php?id_event=' . $valeurs['id_event'] . '');
}
...
if (isset($_POST['reservez'])) {
    ...
    $places = $bdd->prepare('UPDATE places SET statut = "reserve", id_user=:idUser WHERE statut="selected" AND id_zone=3');
    $places->execute(array(
        'idUser' => $idUser
    ));

    header('Location:/ressources/views/commandeZone3.php?id_event=' . $valeurs['id_event'] . '');
    die();
}
