...
if (isset($_POST['envoyer'])) {
    ...
    header('Location:/ressources/views/siegeZone2.php?id_event=' . $valeurs['id_event'] . '');
}
...
if (isset($_POST['reservez'])) {
    ...
    $places = $bdd->prepare('UPDATE places SET statut = "reserve", id_user=:idUser WHERE statut="selected" AND id_zone=2');
    $places->execute(array(
        'idUser' => $idUser
    ));

    header('Location:/ressources/views/commandeZone2.php?id_event=' . $valeurs['id_event'] . '');
    die();
}
